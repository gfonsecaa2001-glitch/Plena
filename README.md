...
- ✅ **Bloqueios de agenda** — períodos sem atendimento (férias, congresso, uma tarde) e intervalo diário de almoço, configurados em `/agendamento`. Valem só para o link público: o agendamento interno segue livre, porque abrir exceção é decisão do nutricionista. O bloqueio é reaplicado na **revalidação do servidor**, então forjar o horário no formulário não fura. No mesmo trecho foi corrigido um risco de sobreposição: horário ocupado passou a ser tratado como **período**, e não como instante — antes, uma consulta marcada à mão às 10:30 não derrubava o encaixe público de 10:00–11:00.
...
- ✅ **Recordatório alimentar** — `/recordatorio/[token]`, preenchido **pelo paciente** no celular, sem login. O nutricionista pede 1 a 7 dias na ficha e manda o link pelo WhatsApp; o paciente escreve em **texto livre** (decisão consciente: obrigar leigo a procurar alimento numa tabela derrubaria a taxa de resposta, e recordatório não preenchido não vale nada). Dá para salvar e voltar depois; ao enviar, o registro vira somente leitura e aparece na ficha, dia a dia, com marcação de "dia fora da rotina". É o único ponto em que alguém sem conta escreve no prontuário — daí as travas: token com validade e revogação, limite por IP (`lib/rate-limit.ts`), `noindex`, e a estrutura (quantos dias, quais refeições) vinda do que está gravado, não do formulário.
